  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 h-[calc(100vh-12rem)] flex flex-col">
      <div className="flex justify-between items-center mb-4 shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">GEOINT Map (Modul 4)</h1>
          <p className="text-slate-500 mt-1">Pemetaan intelijen geospasial untuk pantauan fisik & konsentrasi massa.</p>
        </div>
        <div className="flex items-center gap-3">
          <select className="bg-slate-900 text-white text-sm rounded-lg px-4 py-2 font-medium border-none outline-none shadow-sm">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        {/* Map Area */}
        <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 relative overflow-hidden flex items-center justify-center shadow-inner">
          {/* Dark Map Background */}
        ...
        {/* Sidebar Intel */}
        <div className="w-80 bg-slate-900 border border-slate-800 rounded-xl flex flex-col shrink-0 shadow-lg overflow-hidden text-slate-300">
          <div className="p-4 border-b border-slate-800 bg-black/20 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-white">Log Titik Panas</h3>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {mockHotspots.map((spot, idx) => (
              <div key={idx} className="p-4 rounded-xl border border-slate-800 bg-slate-800/50 relative overflow-hidden">
                <div className={`absolute top-0 left-0 w-1 h-full ${spot.risk === 'Tinggi' ? 'bg-rose-500' : 'bg-amber-500'}`}></div>
                ...
                <p className="text-xs text-slate-400 mb-2 pl-2">{spot.desc}</p>
                <div className="pl-2">
